...
Interactive `console` mode places terminal stdin in raw mode so keystrokes and ANSI replies, including cursor-position reports, reach the Pi immediately without local echo or line buffering. It restores the workstation terminal on exit, keeps `Ctrl-C` as the local exit command, and leaves piped or other non-TTY input unchanged.
